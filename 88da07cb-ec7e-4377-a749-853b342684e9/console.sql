...
if is_increase_with_work_age:
    # 计算社会工龄和公司工龄
    # 社会工龄， 目前是带小数的
    social_work_age = work_age_calculate_func(employee, work_age_count_rule, "social", company_work_age_cross_date, social_work_age_cross_date, period_start_date, period_end_date,datetime, datetime_minus, leave_date=is_resignation_convert_enable and leave_date)
    _logger.info("社会工龄 %s" %social_work_age)
    # 公司工龄，目前是带小数的
    inner_work_age = work_age_calculate_func(employee, work_age_count_rule, "inner", company_work_age_cross_date, social_work_age_cross_date, period_start_date, period_end_date, datetime, datetime_minus,leave_date=is_resignation_convert_enable and leave_date)
...
if is_increase_with_work_age:
    ...
    legal_holiday_count = {}  # 法定年假，如果有跨区间的话：｛"is_cross_year":是否跨区间， "last_num":上个区间的假期天数， "current_num":下个期间的假期天数}，否则last_num和current_num是一致的
    company_holiday_count = {}  # 公司年假
    holiday_count = {}  # 混合年假
    try:
        if is_enable_complex_yos:
            # 使用混合工龄天数计算
            _logger.info("混合年假 %s" %yos.code)
            holiday_count = getattr(rule_obj, yos.code)(inner_work_age, social_work_age)  # 目前是维塔式站用的代码，启用的混合工龄 ‘get_complex_days_count’，是写在代码里的
        else:
            # 使用单一工龄与天数计算
            ...
            if is_enable_legal_annual and not is_enable_company_annual:
                _logger.info("法定年假 %s" %legal_yos.code)
                ...
                _logger.info("法定年假区间规则 %s" %work_age_holiday_num)
                ...
                _logger.info("公司年假 %s" %company_yos.code)
                ...
                _logger.info("2个年假 %s" %legal_yos.code)
...
day_product_uom = get_day_product_uom(employee)  # 获得天类型
...
if frequency == "year":
    ...
    # 法定假期天数
    legal_temp_result = entry_resignation_converter(legal_before_cross_num, legal_current_num, leave_date, period_start_date, period_end_date, social_work_age_cross_date, employee_hire_date, legal_is_cross_year, is_new and is_entry_convert_enable, leave_date and is_resignation_convert_enable)  # 这是hr.holiday.rule.function记录里的python_code函数
    _logger.info("法定假期天数 %s" %legal_temp_result)
    # 公司假期天数
    _logger.info("公司年假跨档 %s" %company_is_cross_year)
    _logger.info("company_work_age_cross_date %s" %company_work_age_cross_date)
    _logger.info("social_work_age_cross_date %s" %social_work_age_cross_date)
    ...
    _logger.info("公司假期天数 %s" %company_temp_result)
    ...
    if is_enable_legal_annual and not is_enable_company_annual:
        ...
        # 法定跟公司都启用
        result = {"social": legal_temp_result, "company": company_temp_result, "start_date": period_start_date, "end_date": period_end_date}  # 年假获得的假期天数是按天来计算的
    ...
    if not is_enable_complex_yos and work_age_count_rule == "convert":  # 不启用混合工龄计算 且为 折算档期
        if legal_is_cross_year:
            ...
            _logger.info("legal_before_cross_num%s" %legal_before_cross_num)
            if social_work_age_cross_date < employee_hire_date:
                legal_current_num = divide(legal_current_num, "month", employee_hire_date, period_end_date, precision, round_rule, copy, datetime, precision_round, monthrange, convert=True, is_entry_convert=is_new and is_entry_convert_enable, is_resignation_convert=leave_date and is_resignation_convert_enable)
            else:
            ...
            _logger.info("legal_current_num%s" %legal_current_num)
...
if employee_hire_date > period_end_date:  # 如果入职日期在区间结束时间之后，则不发假
    result = []
_logger.info("年假发假最终结果 %s" %result)
